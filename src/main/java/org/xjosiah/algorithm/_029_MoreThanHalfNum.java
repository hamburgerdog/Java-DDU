...
public class _029_MoreThanHalfNum {
    ...
    private static final int[] origin = {1, 2, 3, 2, 2, 2, 5, 4, 2};
    ...
    private static int findTheNumByMap(int[] array) {
        ...
        AtomicInteger atomicResult = new AtomicInteger(-1);
        resultMap.keySet().stream()
                .max(Comparator.comparingInt(resultMap::get)).ifPresent(atomicResult::set);

        return resultMap.get(atomicResult.get()) > (array.length / 2) ? atomicResult.get() : Integer.MIN_VALUE;
    }
