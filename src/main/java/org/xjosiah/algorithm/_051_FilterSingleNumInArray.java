 ...
 * @since 2021/3/31
 */
public class _051_FilterSingleNumInArray {
    private static final int N = 8;
    private static final Integer[] originArray = new SplittableRandom().ints()
            .filter(i -> i >= 0)
            .map(i -> i % N)
            .limit(N)
            .boxed()
    ...
    public static void main(String[] args) {
        ...
        int[] input = Arrays.stream(originArray.clone()).mapToInt(Integer::intValue).toArray();
        System.out.println(filterSingleNumInArray(input).toString());
    }
     ...
     * @return 重复数字的集合
     */
    private static HashSet<Integer> filterSingleNumInArray(int[] array) {
