...
public class _018_WhetherIsChildTree {
    private static MyTreeNode<Integer> bigTreeRootNode = new MyTreeNode<>(0);
    private static MyTreeNode<Integer> testTreeRootNode = new MyTreeNode<>(2);
    ...
    private static MyTreeNode<Integer> findBeginNodeInBigTree(MyTreeNode<Integer> node) {
        ...
        if (node.getValue() == testTreeRootNode.getValue()) return node;
    ...
    private static boolean isSameTree(MyTreeNode<Integer> bigTreeNode, MyTreeNode<Integer> testTreeNode) {
        ...
        if (bigTreeNode.getValue() != testTreeNode.getValue()) {
            return false;
        }
